...
@tg_bot.on(events.NewMessage(pattern="/changeUser", incoming=True, from_users=tg_user_name))
@check_in_group
@check_tg_login
@check_od_login
async def change_user_handler(event):
    ...
    if len(cmd) == 2:
        ...
        await event.respond(f'Changed user from\n{old_user}\nto\n{new_user}')
