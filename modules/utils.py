...
class Status_Message:
    ...
    async def create(cls, event):
        ...
        self.event = event
        try:
            self.msg_link = "[Status:](https://t.me/c/%d/%d)" % (
                self.event.message.peer_id.channel_id,
                self.event.message.id,
            )
        except Exception as e:
            logger(e)
            self.msg_link = "[Status:](https://t.me/c/%d/%d)" % (
                self.event.message.peer_id.chat_id,
                self.event.message.id,
            )
...
async def delete_message(message):
    ...
    if env.delete_flag:
        try:
            ...
            await message.reply(
                "Please set this bot as Admin, and give it ability to Delete Messages."
            )
